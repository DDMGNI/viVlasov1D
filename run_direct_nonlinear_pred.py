...
#        OptDB.setValue('log_summary', '')
        ...
        self.h1a = self.da1.createGlobalVec()
        ...
        self.petsc_vlasov = PETScVlasovMatrix(self.da1, self.dax,
                                              self.h0, self.vGrid,
                                              self.nx, self.nv, self.ht, self.hx, self.hv,
...
#        self.poisson_ksp.getPC().setFactorSolverPackage('superlu_dist')
    ...
    def run(self):
        for itime in range(1, self.nt+1):
            ...
            # correct initial guess via linear solvers
            ...
            self.h1h.copy(self.h1a)
            self.h1a.scale(0.5)
            self.h1a.axpy(0.5, self.h1)
            
            self.petsc_vlasov.formMat(self.M, self.h1a)
            self.petsc_vlasov.formRHS(self.fb, self.fh, self.h1a)
            ...
            if PETSc.COMM_WORLD.getRank() == 0:
                print("  Corrector:                          funcnorm = %24.16E" % (corr_norm))
