...
cdef class PETScJacobianMatrixFree(object):
    ...
    def __init__(self, DA da1, DA da2, DA dax, Vec H0,
        ...
        self.NUd = self.dax.createGlobalVec()
        self.NEd = self.dax.createGlobalVec()
        ...
        for i in np.arange(xs, xe):
            ...
            for j in np.arange(0, self.nv):
                if j <= 1 or j >= self.nv-2:
                    ...
                    y[iy, j] = self.toolbox.time_derivative_woa(fd, ix, j) \
                             + 0.5 * self.toolbox.arakawa_J4(fd, h_ave, ix, j) \
                             + 0.5 * self.toolbox.arakawa_J4(f_ave, h1d, ix, j) \
                             - 0.5 * self.nu * self.toolbox.collT1woa(fd, Np, Up, Ep, Ap, ix, j) \
                             - 0.5 * self.nu * self.toolbox.collT2woa(fd, Np, Up, Ep, Ap, ix, j) \
                             - 0.5 * coll1_fac * ( fp[ix, j+1] - fp[ix, j-1] ) * Ud[ix] * Ap[ix] \
                             + 0.5 * coll1_fac * ( fp[ix, j+1] * ( self.v[j+1] - Up[ix] ) - fp[ix, j-1] * ( self.v[j-1] - Up[ix] ) ) * 2. * Ud[ix] * Up[ix] * Ap[ix]**2 \
                             - 0.5 * coll1_fac * ( fp[ix, j+1] * ( self.v[j+1] - Up[ix] ) - fp[ix, j-1] * ( self.v[j-1] - Up[ix] ) )      * Ed[ix]          * Ap[ix]**2
