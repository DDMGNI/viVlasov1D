...
#         cdef np.float64_t coll2_fac = - 4.0 *  0.5 * self.nu * 0.25 * self.hv2_inv
        ...
        cdef np.float64_t coll1_fac = - 4.0 *  0.5 * self.nu * 0.5 / self.hv
        cdef np.float64_t coll2_fac = - 4.0 *  0.5 * self.nu * self.hv2_inv
        ...
        for i in np.arange(xs, xe):
            ...
            col.field = self.nv+1
            A.setValueStencil(row, col, Up[ix])
            
            col.field = self.nv+2
            A.setValueStencil(row, col, Np[ix])
            ...
            col.field = self.nv+1
            A.setValueStencil(row, col, Ep[ix])
...
#                                                  - 1. * coll1_fac * fp[ix+1, j-1] * ( Np[ix+1] * v[j-1] - Up[ix+1] ) * Ap[ix+1]**2 * (-1.) ),
                    ...
                    for index, field, value in [
                            ...
                            ((i,  ), j-1, 2. * time_fac_J1 + (h_ave[ix+1, j  ] - h_ave[ix-1, j  ]) * arak_fac_J1 \
                                                           + (h_ave[ix+1, j-1] - h_ave[ix-1, j-1]) * arak_fac_J1 \
                                                           - coll1_fac * ( v[j-1] - Up[ix  ] ) \
                                                           + coll2_fac * ( Ep[ix] - Up[ix  ]**2 )),
                            ((i,  ), j  , 4. * time_fac_J1 \
                                                           - 2. * coll2_fac * ( Ep[ix] - Up[ix  ]**2 )),
                            ((i,  ), j+1, 2. * time_fac_J1 - (h_ave[ix+1, j  ] - h_ave[ix-1, j  ]) * arak_fac_J1 \
                                                           - (h_ave[ix+1, j+1] - h_ave[ix-1, j+1]) * arak_fac_J1 \
                                                           + coll1_fac * ( v[j+1] - Up[ix  ] ) \
                                                           + coll2_fac * ( Ep[ix] - Up[ix  ]**2 )),
                            ...
                            ((i,  ), self.nv+2,  - coll1_fac * fp[ix,   j+1] \
                                                 + coll1_fac * fp[ix,   j-1] \
                                                 - 1. * coll2_fac * fp[ix,   j+1] * 2. * Up[ix  ] \
                                                 + 2. * coll2_fac * fp[ix,   j  ] * 2. * Up[ix  ] \
                                                 - 1. * coll2_fac * fp[ix,   j-1] * 2. * Up[ix  ] ),
                            
                            ((i,  ), self.nv+3,  + 1. * coll2_fac * fp[ix,   j+1] \
                                                 - 2. * coll2_fac * fp[ix,   j  ] \
                                                 + 1. * coll2_fac * fp[ix,   j-1] ),
                        ]:
