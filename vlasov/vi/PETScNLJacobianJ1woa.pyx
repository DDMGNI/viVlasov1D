...
#         cdef np.float64_t coll2_fac   = 0.
        ...
        cdef np.float64_t time_fac_J1 = 4.0 *  1.0 / self.ht
                ...
                if j <= 1 or j >= self.nv-2:
                    ...
                    for index, field, value in [
                            ...
                            ((i,  ), j-1,             + (h_ave[ix+1, j  ] - h_ave[ix-1, j  ]) * arak_fac_J1 \
                                                      + (h_ave[ix+1, j-1] - h_ave[ix-1, j-1]) * arak_fac_J1 \
                                                      - coll1_fac * ( Np[ix  ] * v[j-1] - Up[ix  ] ) * Ap[ix  ] \
                                                      + coll2_fac),
                            ((i,  ), j  , time_fac_J1 - 2. * coll2_fac),
                            ((i,  ), j+1,             - (h_ave[ix+1, j  ] - h_ave[ix-1, j  ]) * arak_fac_J1 \
                                                      - (h_ave[ix+1, j+1] - h_ave[ix-1, j+1]) * arak_fac_J1 \
                                                      + coll1_fac * ( Np[ix  ] * v[j+1] - Up[ix  ] ) * Ap[ix  ] \
                                                      + coll2_fac),
                            ...
                            ((i,  ), self.nv+3,  + coll1_fac * fp[ix,   j+1] * ( Np[ix  ] * v[j+1] - Up[ix  ] ) * Ap[ix  ]**2 * (-1.) \
                                                 - coll1_fac * fp[ix,   j-1] * ( Np[ix  ] * v[j-1] - Up[ix  ] ) * Ap[ix  ]**2 * (-1.) ),
                        ]:
