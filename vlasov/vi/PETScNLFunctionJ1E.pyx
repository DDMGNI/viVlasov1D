...
cdef class PETScFunction(object):
    ...
    def matrix_mult(self, Vec Y):
        ...
        for i in np.arange(xs, xe):
            ...
            y[iy, self.nv+2] = Np[ix] * Up[ix] - (fp[ix] * self.v   ).sum() * self.hv 
            y[iy, self.nv+3] = Np[ix] * Ep[ix] - (fp[ix] * self.v**2).sum() * self.hv
            ...
            for j in np.arange(0, self.nv):
                if j <= 1 or j >= self.nv-2:
                    ...
                    y[iy, j] = 4.0 * ( \
                                       ...
                                       - 0.5 * self.nu * self.toolbox.collE1(fp, Np, Up, Ep, Ap, ix, j) \
                                       - 0.5 * self.nu * self.toolbox.collE1(fh, Nh, Uh, Eh, Ah, ix, j) \
                                       - 0.5 * self.nu * self.toolbox.collE2(fp, Np, Up, Ep, Ap, ix, j) \
                                       - 0.5 * self.nu * self.toolbox.collE2(fh, Nh, Uh, Eh, Ah, ix, j)
                                     )
