            ...
            for j in range(ys, ye):
                ...
                if j <= 1 or j >= self.grid.nv-2:
                    ...
                    for index, field, value in [
                            ...
                            ((i-1,), j-1, - (hh[ix-2, jx  ] - hh[ix,   jx-2]) * arak_fac_J2 \
                                          - (hh[ix-1, jx+1] - hh[ix+1, jx-1]) * arak_fac_J2),
                            ((i-1,), j+1, - (hh[ix,   jx+2] - hh[ix-2, jx  ]) * arak_fac_J2 \
                                          - (hh[ix+1, jx+1] - hh[ix-1, jx-1]) * arak_fac_J2),
                            ...
                            ((i-2,), self.grid.nv, + (fh[ix-1, jx+1] - fh[ix-1, jx-1]) * arak_fac_J2),
                            ((i-1,), self.grid.nv, + (fh[ix-1, jx+1] - fh[ix+1, jx-1]) * arak_fac_J2 \
                                              + (fh[ix+1, jx+1] - fh[ix-1, jx-1]) * arak_fac_J2 \
                                              + (fh[ix-2, jx  ] - fh[ix,   jx-2]) * arak_fac_J2 \
                                              + (fh[ix,   jx+2] - fh[ix-2, jx  ]) * arak_fac_J2),
                            ((i,  ), self.grid.nv, + (fh[ix+1, jx+1] - fh[ix-1, jx+1]) * arak_fac_J2 \
                                              - (fh[ix+1, jx-1] - fh[ix-1, jx-1]) * arak_fac_J2),
                            ((i+1,), self.grid.nv, - (fh[ix-1, jx+1] - fh[ix+1, jx-1]) * arak_fac_J2 \
                                              - (fh[ix+1, jx+1] - fh[ix-1, jx-1]) * arak_fac_J2 \
                                              - (fh[ix,   jx+2] - fh[ix+2, jx  ]) * arak_fac_J2 \
            ...
            for j in range(ys, ye):
                ...
                if j <= 1 or j >= self.grid.nv-2:
                    ...
                    y[iy, jy] = self.toolbox.time_derivative(fd, ix, j) \
                             + 0.5 * self.toolbox.arakawa_J2(fd, hh, ix, j) \
                             + 0.5 * self.toolbox.arakawa_J2(fh, hd, ix, j) \
            ...
            for j in range(ys, ye):
                ...
                if j <= 1 or j >= self.grid.nv-2:
                    ...
                    y[iy, jy] = self.toolbox.time_derivative(self.fd, ix, j) \
                             ...
                             + 0.5 * self.toolbox.arakawa_J2(fp, hh, ix, j) \
                             + 0.5 * self.toolbox.arakawa_J2(fh, hp, ix, j) #\
