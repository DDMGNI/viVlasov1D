        ...
        for i in range(xs, xe):
            ...
            for j in range(0, self.nv):
                if j < self.da1.getStencilWidth() or j >= self.nv-self.da1.getStencilWidth():
                    ...
                    result = ( \
                                 + 3 * fd[ix,   j+1] * h_ave[ix+1, j+1] \
                                 - 3 * fd[ix,   j-1] * h_ave[ix+1, j  ] \
                                 - 3 * fd[ix-1, j  ] * h_ave[ix-1, j-1] \
                                 + 3 * fd[ix-1, j-1] * h_ave[ix-1, j  ] \
                                 - 3 * fd[ix+1, j+1] * h_ave[ix,   j+1] \
                                 + 3 * fd[ix+1, j  ] * h_ave[ix,   j-1] \
                               \
        ...
        for i in range(xs, xe):
            ...
            for j in range(0, self.nv):
                if j < self.da1.getStencilWidth() or j >= self.nv-self.da1.getStencilWidth():
                    ...
                    result = ( \
                                 + 3 * f_ave[ix,   j+1] * h_ave[ix+1, j+1] \
                                 - 3 * f_ave[ix,   j-1] * h_ave[ix+1, j  ] \
                                 - 3 * f_ave[ix-1, j  ] * h_ave[ix-1, j-1] \
                                 + 3 * f_ave[ix-1, j-1] * h_ave[ix-1, j  ] \
                                 - 3 * f_ave[ix+1, j+1] * h_ave[ix,   j+1] \
                                 + 3 * f_ave[ix+1, j  ] * h_ave[ix,   j-1] \
                               \
