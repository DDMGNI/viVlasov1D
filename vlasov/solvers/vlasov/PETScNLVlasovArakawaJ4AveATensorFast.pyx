        ...
        for j in range(ys, ye):
            ...
            if j < self.grid.stencil or j >= self.grid.nv-self.grid.stencil:
                ...
                for i in range(xs, xe):
                    ...
                    # time derivative
                    time_deriv = ( 0. * fd[ix-2, jx-2] + 0. * fd[ix-1, jx-2] + 1. * fd[ix, jx-2] + 0. * fd[ix+1, jx-2] + 0. * fd[ix+2, jx-2]
                                 + 0. * fd[ix-2, jx-1] + 2. * fd[ix-1, jx-1] + 8. * fd[ix, jx-1] + 2. * fd[ix+1, jx-1] + 0. * fd[ix+2, jx-1]
                                 + 1. * fd[ix-2, jx  ] + 8. * fd[ix-1, jx  ] +20. * fd[ix, jx  ] + 8. * fd[ix+1, jx  ] + 1. * fd[ix+2, jx  ]
                                 + 0. * fd[ix-2, jx+1] + 2. * fd[ix-1, jx+1] + 8. * fd[ix, jx+1] + 2. * fd[ix+1, jx+1] + 0. * fd[ix+2, jx+1]
                                 + 0. * fd[ix-2, jx+2] + 0. * fd[ix-1, jx+2] + 1. * fd[ix, jx+2] + 0. * fd[ix+1, jx+2] + 0. * fd[ix+2, jx+2]
                                 ) * self.grid.ht_inv / 64.
