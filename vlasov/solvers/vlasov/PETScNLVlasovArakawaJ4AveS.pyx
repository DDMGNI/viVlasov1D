        ...
        for j in range(ys, ye):
            ...
            if j < self.grid.stencil or j >= self.grid.nv-self.grid.stencil:
                ...
                for i in range(xs, xe):
                    ...
                    # time derivative
                    time_deriv = ( 1. * fd[ix-1, jx-1] + 4. * fd[ix, jx-1] + 1. * fd[ix+1, jx-1]
                                 + 4. * fd[ix-1, jx  ] +16. * fd[ix, jx  ] + 4. * fd[ix+1, jx  ]
                                 + 1. * fd[ix-1, jx+1] + 4. * fd[ix, jx+1] + 1. * fd[ix+1, jx+1] ) * self.grid.ht_inv / 36.
        ...
        for j in range(ys, ye):
            ...
            if j < self.grid.stencil or j >= self.grid.nv-self.grid.stencil:
                ...
                for i in range(xs, xe):
                    ...
                    # time derivative
                    time_deriv_p = ( 1. * fp[ix-1, jx-1] + 4. * fp[ix, jx-1] + 1. * fp[ix+1, jx-1]
                                   + 4. * fp[ix-1, jx  ] +16. * fp[ix, jx  ] + 4. * fp[ix+1, jx  ]
                                   + 1. * fp[ix-1, jx+1] + 4. * fp[ix, jx+1] + 1. * fp[ix+1, jx+1] ) * self.grid.ht_inv / 36.
                    
                    time_deriv_h = ( 1. * fh[ix-1, jx-1] + 4. * fh[ix, jx-1] + 1. * fh[ix+1, jx-1]
                                   + 4. * fh[ix-1, jx  ] +16. * fh[ix, jx  ] + 4. * fh[ix+1, jx  ]
                                   + 1. * fh[ix-1, jx+1] + 4. * fh[ix, jx+1] + 1. * fh[ix+1, jx+1] ) * self.grid.ht_inv / 36.
