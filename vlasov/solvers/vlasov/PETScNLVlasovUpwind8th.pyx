        ...
        for i in range(xs, xe):
            ...
            for j in range(0, self.nv):
                if j < self.da1.getStencilWidth() or j >= self.nv-self.da1.getStencilWidth():
                    ...
                    result = \
                             - 1. * fd[ix+1, j-1] * h_ave[ix+1, j+1] \
                             + 1. * fd[ix,   j+2] * h_ave[ix+1, j+1] \
                             + 1. * fd[ix+1, j+1] * h_ave[ix+1, j-1] \
                             - 1. * fd[ix,   j-2] * h_ave[ix+1, j-1] \
                             + 1. * fd[ix-1, j-1] * h_ave[ix-1, j+1] \
                             - 1. * fd[ix,   j+2] * h_ave[ix-1, j+1] \
                             - 1. * fd[ix-1, j+1] * h_ave[ix-1, j-1] \
                             + 1. * fd[ix,   j-2] * h_ave[ix-1, j-1] \
                             - 1. * fd[ix+1, j+1] * h_ave[ix,   j+2] \
                             + 1. * fd[ix-1, j+1] * h_ave[ix,   j+2] \
                             + 1. * fd[ix+1, j-1] * h_ave[ix,   j-2] \
                             - 1. * fd[ix-1, j-1] * h_ave[ix,   j-2] \
                             + 2. * fd[ix+1, j  ] * h_ave[ix+1, j+1] \
                             - 2. * fd[ix,   j+1] * h_ave[ix+1, j+1] \
                             - 2. * fd[ix+1, j  ] * h_ave[ix+1, j-1] \
                             + 2. * fd[ix,   j-1] * h_ave[ix+1, j-1] \
                             - 2. * fd[ix+1, j+1] * h_ave[ix+1, j  ] \
                             + 2. * fd[ix+1, j-1] * h_ave[ix+1, j  ] \
                             - 2. * fd[ix,   j+1] * h_ave[ix+1, j  ] \
                             + 2. * fd[ix,   j-1] * h_ave[ix+1, j  ] \
                             - 2. * fd[ix-1, j  ] * h_ave[ix-1, j+1] \
                             + 2. * fd[ix,   j+1] * h_ave[ix-1, j+1] \
                             + 2. * fd[ix-1, j  ] * h_ave[ix-1, j-1] \
                             ...
                             + 2. * fd[ix,   j+1] * h_ave[ix-1, j  ] \
                             - 2. * fd[ix,   j-1] * h_ave[ix-1, j  ] \
                             + 2. * fd[ix+1, j+1] * h_ave[ix,   j+1] \
                             + 2. * fd[ix+1, j  ] * h_ave[ix,   j+1] \
                             - 2. * fd[ix-1, j+1] * h_ave[ix,   j+1] \
                             - 2. * fd[ix-1, j  ] * h_ave[ix,   j+1] \
                             - 2. * fd[ix+1, j-1] * h_ave[ix,   j-1] \
                             - 2. * fd[ix+1, j  ] * h_ave[ix,   j-1] \
                    ...
                    y[iy, j] = fd[ix, j] * self.ht_inv \
                             + 0.5 * result  * self.hx_inv * self.hv_inv / 12. \
        ...
        for i in range(xs, xe):
            ...
            for j in range(0, self.nv):
                if j < self.da1.getStencilWidth() or j >= self.nv-self.da1.getStencilWidth():
                    ...
                    result = \
                             - 1. * f_ave[ix+1, j-1] * h_ave[ix+1, j+1] \
                             + 1. * f_ave[ix,   j+2] * h_ave[ix+1, j+1] \
                             + 1. * f_ave[ix+1, j+1] * h_ave[ix+1, j-1] \
                             - 1. * f_ave[ix,   j-2] * h_ave[ix+1, j-1] \
                             + 1. * f_ave[ix-1, j-1] * h_ave[ix-1, j+1] \
                             - 1. * f_ave[ix,   j+2] * h_ave[ix-1, j+1] \
                             - 1. * f_ave[ix-1, j+1] * h_ave[ix-1, j-1] \
                             + 1. * f_ave[ix,   j-2] * h_ave[ix-1, j-1] \
                             - 1. * f_ave[ix+1, j+1] * h_ave[ix,   j+2] \
                             + 1. * f_ave[ix-1, j+1] * h_ave[ix,   j+2] \
                             + 1. * f_ave[ix+1, j-1] * h_ave[ix,   j-2] \
                             - 1. * f_ave[ix-1, j-1] * h_ave[ix,   j-2] \
                             + 2. * f_ave[ix+1, j  ] * h_ave[ix+1, j+1] \
                             - 2. * f_ave[ix,   j+1] * h_ave[ix+1, j+1] \
                             - 2. * f_ave[ix+1, j  ] * h_ave[ix+1, j-1] \
                             + 2. * f_ave[ix,   j-1] * h_ave[ix+1, j-1] \
                             - 2. * f_ave[ix+1, j+1] * h_ave[ix+1, j  ] \
                             + 2. * f_ave[ix+1, j-1] * h_ave[ix+1, j  ] \
                             - 2. * f_ave[ix,   j+1] * h_ave[ix+1, j  ] \
                             + 2. * f_ave[ix,   j-1] * h_ave[ix+1, j  ] \
                             - 2. * f_ave[ix-1, j  ] * h_ave[ix-1, j+1] \
                             + 2. * f_ave[ix,   j+1] * h_ave[ix-1, j+1] \
                             + 2. * f_ave[ix-1, j  ] * h_ave[ix-1, j-1] \
                             ...
                             + 2. * f_ave[ix,   j+1] * h_ave[ix-1, j  ] \
                             - 2. * f_ave[ix,   j-1] * h_ave[ix-1, j  ] \
                             + 2. * f_ave[ix+1, j+1] * h_ave[ix,   j+1] \
                             + 2. * f_ave[ix+1, j  ] * h_ave[ix,   j+1] \
                             - 2. * f_ave[ix-1, j+1] * h_ave[ix,   j+1] \
                             - 2. * f_ave[ix-1, j  ] * h_ave[ix,   j+1] \
                             - 2. * f_ave[ix+1, j-1] * h_ave[ix,   j-1] \
                             - 2. * f_ave[ix+1, j  ] * h_ave[ix,   j-1] \
                    ...
                    y[iy, j] = (fp[ix, j] - fh[ix, j]) * self.ht_inv \
                             + 1.0 * result * self.hx_inv * self.hv_inv / 12. \
                             - 0.5 * self.nu * self.coll_drag * coll_drag * self.hv_inv * 0.5 \
                             - 0.5 * self.nu * self.coll_diff * coll_diff * self.hv2_inv
