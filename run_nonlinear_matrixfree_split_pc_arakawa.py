...
import vlasov.predictor.PETScNLVlasovArakawaJ1
...
class petscVP1Dmatrixfree(petscVP1Dbase):
    ...
    def updateArakawaJacobian(self, snes, X, J, P):
        self.petsc_arakawa_solver.update_delta(X)
        
        if J != P:
            self.petsc_arakawa_solver.formJacobian(P)
...
#         self.Jpc.setUp()
        ...
        self.petsc_arakawa_solver = vlasov.predictor.PETScNLVlasovArakawaJ1.PETScVlasovSolver(
        ...
        self.Jmf_arakawa = PETSc.Mat().createPython([self.f.getSizes(), self.bpred.getSizes()], 
                                                   context=self.petsc_arakawa_solver,
                                                   comm=PETSc.COMM_WORLD)
        self.Jmf_arakawa.setUp()
        
        

        # copy external potential
        self.petsc_vlasov_solver.update_external(self.p_ext)
        self.petsc_arakawa_solver.update_external(self.p_ext)
        self.petsc_solver.update_external(self.p_ext)
        
        # update solution history
        self.petsc_vlasov_solver.update_history(self.x)
        self.petsc_arakawa_solver.update_history(self.x)
        ...
        self.snes_pred.setFunction(self.petsc_arakawa_solver.function_snes_mult, self.bpred)
        self.snes_pred.setJacobian(self.updateArakawaJacobian, self.Jmf_arakawa)
...
#                 print("                                      sum(phi) = %24.16E" % (phisum))
            ...
            self.petsc_arakawa_solver.update_previous(self.x)
            ...
            if PETSc.COMM_WORLD.getRank() == 0:
                print("  Arakawa Solver:   %5i GMRES  iterations, funcnorm = %24.16E" % (self.snes_pred.getLinearSolveIterations(), pred_norm) )
