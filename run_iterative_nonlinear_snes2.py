...
#        self.poisson_ksp.setNullSpace(self.poisson_nsp)        
        ...
        OptDB.setValue('snes_max_it', 1)
        OptDB.setValue('ksp_max_it',  3)
        OptDB.setValue('snes_convergence_test', 'skip')
        OptDB.setValue('ksp_convergence_test',  'skip')
        
        # create nonlinear solver
        self.snes_gmres = PETSc.SNES().create()
        self.snes_gmres.setFunction(self.petsc_function.snes_mult, self.F)
        self.snes_gmres.setJacobian(self.updateJacobianMF, self.Jmf)
        self.snes_gmres.setFromOptions()
        self.snes_gmres.getKSP().setType('gmres')
        self.snes_gmres.getKSP().getPC().setType('none')
    ...
    def run(self):
        for itime in range(1, self.nt+1):
            ...
            while True:
                ...
                # output some solver info
                if PETSc.COMM_WORLD.getRank() == 0:
                    print("  Nonlin Precon:  %5i iterations,   funcnorm = %24.16E" % (i, fnorm) )
                
                if fnorm < self.cfg['solver']['petsc_snes_atol'] or i >= 10:
                    break
            
#            # nonlinear solve
#            self.snes_gmres.solve(None, self.x)
#            
#            # output some solver info
#            if PETSc.COMM_WORLD.getRank() == 0:
#                print()
#                print("  Nonlin Solver:  %5i iterations,   funcnorm = %24.16E" % (self.snes_gmres.getIterationNumber(), self.snes_gmres.getFunctionNorm()) )
#                print()
#            
#            if self.snes_gmres.getConvergedReason() < 0:
#                if PETSc.COMM_WORLD.getRank() == 0:
#                    print()
#                    print("Solver not converging...   %i" % (self.snes_gmres.getConvergedReason()))
#                    print()
