...
from vlasov.vi import PETScSimpleNLFunctionCollD
from vlasov.vi import PETScSimpleNLJacobianCollD
...
#        OptDB.setValue('ksp_max_it',  100)
        ...
        OptDB.setValue('snes_lag_preconditioner', 20)
        OptDB.setValue('snes_ksp_ew_conv',  '')
...
#        OptDB.setValue('log_summary', '')
        ...
        self.petsc_jacobian = PETScSimpleNLJacobianCollD.PETScJacobian(
                                            self.da1, self.da2, self.dax,
                                            self.h0, self.vGrid,
                                            self.nx, self.nv, self.ht, self.hx, self.hv,
                                            self.charge, coll_freq=self.coll_freq)
        
        self.petsc_function = PETScSimpleNLFunctionCollD.PETScFunction(
                                            self.da1, self.da2, self.dax, 
                                            self.h0, self.vGrid,
                                            self.nx, self.nv, self.ht, self.hx, self.hv,
                                            self.charge, coll_freq=self.coll_freq)
...
#        OptDB.setValue('ksp_rtol',    1E-7)
        ...
        self.snes_nonlinear.setFunction(self.petsc_function_mf.snes_mult, self.F)
...
#        OptDB.setValue('ksp_gmres_cgs_refinement_type',  'ifneeded')


        # create nonlinear solver with collisions
        self.snes_collisions = PETSc.SNES().create()
        self.snes_collisions.setFunction(self.petsc_function_mf.snes_mult, self.F)
